...
class EmbeddingGradientIntegralDriver:
    ...
    def electronic_field_nuclear_gradients(self,
        ...
        Not to be confused with `electronic_field_gradients`, which
        IntegralDriverTemplate reserves for the field gradient tensor at each
        site -- a different quantity with a different shape. Hence the name.
        ...
        of the field as QuantumSubsystem.compute_electronic_fields returns it,
        the sign of the electron charge included, so that it can be added to
        the nuclear field gradients and handed to the dipole solver.
        """
        mol = self.mol
        nao = mol.nao
...
def perturbed_induced_dipoles(embedding_obj, dm):
    ...
    # compute_nuclear_fields -- PyFraME's induction gradient consumes it with
    # that sign built in -- so it is negated here to be added to the electronic
    # part, which carries the sign of the field itself.
    ...
    # as in _B_dot_x: the solver keeps every set it produces to start later ones
    # from, and none of these is ever reused, so the subsystem is left with the
    # entries it had
    cache = classical_subsystem.perturbed_induced_dipoles
    n_cached = len(cache)
...
def _potential_interaction_gradient(embedding_obj, difference, frozen_dm):
    ...
    if embedding_obj.with_induction:
        # the potential carries -f_el_ind, so its derivative does too
