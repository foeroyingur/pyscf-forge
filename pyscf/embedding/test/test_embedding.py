...
try:
    # Probe pyframe.embedding rather than `import pyframe`, which succeeds
    # against released PyFraME 0.4.0, where the subpackage does not exist.
    import pyframe.embedding  # noqa: F401
    has_pyframe = True
except ImportError:
    has_pyframe = False

# Not guarded: a PyFraME that has pyframe.embedding but is too old for this
# module has to fail the suite, with the ImportError that says why, rather
# than skip it.
if has_pyframe:
    import pyframe
...
def write_potential(potential, name):
    path = os.path.join(tmpdir, name)
    with open(path, 'w') as handle:
        json.dump(potential, handle)
    return path
...
def make_pyframe_system(name='butadiene_water'):
    '''Butadiene and the three waters around it, as a PyFraME system.

    The same structure as the bundled potential, written as a PDB and
    partitioned by PyFraME, with the waters described by the SEP standard
    potential -- a table lookup, so no quantum chemistry program is run.
    '''
    potential = load_potential()
    nuclei = potential['quantum_subsystems'][0]['nuclei']
    atoms = potential['classical_subsystems'][0]['classical_fragments'][0]['atoms']
    records = []
    counts = {}
    for nucleus in nuclei:
        ...
        records.append(('%s%d' % (element, counts[element]), 'BUT', 1,
                        nucleus['coordinate'], element))
    for i, atom in enumerate(atoms):
        records.append((('O', 'H1', 'H2')[i % 3], 'WAT', 2 + i // 3,
                        atom['coordinate'], atom['element']))
    lines = []
    for serial, (atom_name, residue, resid, coordinate, element) in enumerate(records, 1):
        x, y, z = np.asarray(coordinate) * BOHR
        lines.append('HETATM%5d %-4s %3s A%4d    %8.3f%8.3f%8.3f  1.00  0.00'
                     '          %2s' % (serial, atom_name, residue, resid,
                                        x, y, z, element))
    lines.append('END')
    work_dir = tempfile.mkdtemp(dir=tmpdir)
    pdb = os.path.join(work_dir, name + '.pdb')
...
class TestPolarizableEmbedding(unittest.TestCase):
    ...
    def test_dft_matches_unrestricted_dft(self):
        ...
        # The embedding energy is linear in the density, where the total energy
        # is stationary in it, so the 4e-8 the two converged densities differ by
        # reaches it: 2e-9, which is the SCF's convergence and not the path's.
        # The paths are compared on one density instead, where what is left is
        # the two models solving their dipoles from the ones their own SCF left
        # them: 1e-12 at the default threshold of 1e-8.
        dm = rks.make_rdm1()
        self.assertAlmostEqual(uks.with_embedding.kernel(dm)[0],
                               rks.with_embedding.kernel(dm)[0], 11)
    ...
    def test_option_validation(self):
        ...
        with self.assertRaises(ValueError):
            # the key the potential used to be given under
            embedding.polarizable(mol, {'json_file': json_file})
        with self.assertRaises(TypeError):
            # a potential that is none
            embedding.polarizable(mol, {'potential': 1.0})
        with self.assertRaises(ValueError):
            embedding.polarizable(mol, {'potential': json_file, 'typo': 1})
        with self.assertRaises(ValueError):
            embedding.polarizable(mol, {'potential': json_file, 'vdw': {'methd': 'LJ'}})
        with self.assertRaises(ValueError):
            embedding.polarizable(mol, {'potential': json_file,
                                        'induced_dipoles': {'tolerance': 1e-8}})
        with self.assertRaises(TypeError):
            embedding.polarizable(mol, {'potential': json_file, 'vdw': 'LJ'})
        with self.assertRaises(TypeError):
            # no potential at all
    ...
    def test_vdw_energy_matches_explicit_lennard_jones(self):
        ...
        mf = embedding.polarizable(scf.RHF(mol),
                                   {'potential': potential, 'vdw': {'method': 'LJ'}})
        mf.conv_tol = 1e-12
        mf.kernel()
        with_embedding = mf.with_embedding
    ...
    def test_vdw_gradient_matches_explicit_lennard_jones(self):
        ...
        potential = make_vdw_potential()
        mol = butadiene()
        # The default orbital gradient threshold of sqrt(conv_tol) = 1e-6 does
        # not converge the densities far enough to compare them, hence the
        # tighter one. Even at 1e-9, one more SCF cycle moves them by 2e-9, and
        # whether the energy criterion stops the SCF a cycle earlier is decided
        # by its last digits, so from run to run they have been seen both
        # within 1e-9 and at 2e-9 of each other. 1e-8 does not depend on which.
        with_vdw = embedding.polarizable(scf.RHF(mol),
                                         {'potential': potential, 'vdw': {'method': 'LJ'}})
        with_vdw.conv_tol = 1e-12
        with_vdw.conv_tol_grad = 1e-9
        with_vdw.kernel()
        without_vdw = embedding.polarizable(scf.RHF(mol), potential)
        without_vdw.conv_tol = 1e-12
        without_vdw.conv_tol_grad = 1e-9
        without_vdw.kernel()

        self.assertAlmostEqual(
            abs(with_vdw.make_rdm1() - without_vdw.make_rdm1()).max(), 0, 8)
    ...
    def test_vdw_environment_energy(self):
        ...
        with_vdw = embedding.polarizable(mol, {'potential': potential,
                                               'vdw': {'method': 'LJ'}})
    ...
    def test_vdw_with_partial_parameters(self):
        ...
        full = embedding.polarizable(mol, {'potential': make_vdw_potential(),
                                           'vdw': {'method': 'LJ'}})
    ...
    def test_vdw_combination_rules(self):
        ...
        for rule in ('Lorentz-Berthelot', 'Lorentz-Fender-Halsey',
            ...
            obj = embedding.polarizable(mol, {'potential': potential,
                                              'vdw': {'method': 'LJ',
    ...
    def test_vdw_with_an_unknown_combination_rule_is_refused(self):
        ...
        mol = butadiene()
        with self.assertRaises(NotImplementedError):
            embedding.polarizable(mol, {'potential': make_vdw_potential(),
                                        'vdw': {'method': 'LJ',
    ...
    def test_vdw_with_inconsistent_exponents_is_refused(self):
        ...
        with self.assertRaises(ValueError) as caught:
            embedding.polarizable(mol, {'potential': mixed, 'vdw': {'method': 'LJ'}})
    ...
    def test_vdw_without_any_parameters_is_refused(self):
        ...
        mol = butadiene()
        with self.assertRaises(ValueError) as caught:
            embedding.polarizable(mol, {'potential': json_file, 'vdw': {'method': 'LJ'}})
    ...
    def _converged_rhf(self):
        mf = embedding.polarizable(
            scf.RHF(butadiene()),
            {'potential': json_file, 'induced_dipoles': {'threshold': 1e-12}})
        mf.conv_tol = 1e-12
        mf.kernel()
        return mf
    ...
    def test_frozen_potential_still_follows_the_nuclei(self):
        ...
        # The frozen model solves its dipoles from the ones it had at the
        # geometry before, and the reference from the fields alone, so the two
        # stop within the dipole threshold of each other: 5e-12 at the default
        # of 1e-8, and 4e-16 at 1e-12.
        self.assertAlmostEqual(frozen._e_frozen, e_reference, 11)
        self.assertAlmostEqual(abs(v_moved - v_reference).max(), 0, 10)
    ...
    def test_frozen_post_scf_is_the_plain_method_on_the_embedded_scf(self):
        ...
        # The two paths differ by 1.7e-10 however tightly the induced dipoles are
        # converged, measured down to a threshold of 1e-12 and with PyFraME's
        # older solvers too; at the old default it was masked to 1.6e-11 by a
        # solver error of the opposite sign.
        self.assertAlmostEqual(e_corr, mp.MP2(mf).kernel()[0], 9)
    ...
    def test_quantum_subsystem_is_the_terminated_core_region(self):
        '''A link atom is a nucleus of the potential, so the molecule needs it.

        The last hydrogen is relabelled a link atom and the last carbon a
        capping atom. The molecule without them is refused, naming them; with
        them the potential is taken, and a capping atom that carries no ECP
        in the molecule is warned about.
        '''
        potential = load_potential()
        nuclei = potential['quantum_subsystems'][0]['nuclei']
        nuclei[-1]['kind'] = 'link'
        nuclei[3]['kind'] = 'cap'
        path = write_potential(potential, 'terminated.json')

        truncated = butadiene(atom='\n'.join(BUTADIENE.strip().split('\n')[:-1]),
                              spin=1)
        with self.assertRaisesRegex(ValueError, '1 link atoms and 1 capping'):
            embedding.polarizable(truncated, path)

        mol = butadiene(verbose=4)
        mol.stdout = io.StringIO()
        embedding.polarizable(mol, path)
        self.assertIn('capping atom', mol.stdout.getvalue())

    def test_nuclei_interact_through_their_effective_charges(self):
        '''An atom with an ECP interacts with the environment through Z - ncore.

        Its core electrons are not in the density, so a nuclear-electrostatic
        term with the full nuclear charge would count the attraction of the
        environment to charge that is not there. Checked against the charges
        of the molecule directly, on a charges-only potential.
        '''
        potential = make_potential('ecp_charges.json', orders=lambda i: 0,
                                   depolarize=True)
        mol = butadiene(basis={'C': 'bfd-vdz', 'H': 'sto3g'},
                        ecp={'C': 'bfd-pp'})
        self.assertEqual(mol.atom_charges()[0], 4)

        model = embedding.electrostatic(mol, potential)
        np.testing.assert_array_equal(model.quantum_subsystem.charges,
                                      mol.atom_charges())
        coordinates, charges = point_charges()
        distances = np.linalg.norm(mol.atom_coords()[:, None]
                                   - coordinates[None], axis=-1)
        expected = np.einsum('a,s,as->', mol.atom_charges(), charges,
                             1 / distances)
        self.assertAlmostEqual(model._e_nuc_es, expected, 10)

        # and the energy agrees with point-charge embedding in pyscf.qmmm
        ...
        mm.conv_tol = 1e-11
        mm.kernel()
        self.assertAlmostEqual(es.e_tot, mm.e_tot, 9)
        self.assertAlmostEqual(
            abs(es.nuc_grad_method().kernel()
                - mm.nuc_grad_method().kernel()).max(), 0, 7)

    def test_ghost_atoms_do_not_interact(self):
        '''A ghost atom has no nucleus, and is still the element it stands for.'''
        lines = BUTADIENE.strip().split('\n')
        lines[4] = 'ghost-' + lines[4].split(None, 1)[0] + ' ' + lines[4].split(None, 1)[1]
        mol = butadiene(atom='\n'.join(lines), spin=1, verbose=4)
        mol.stdout = io.StringIO()
        model = embedding.electrostatic(mol, json_file)
        self.assertEqual(model.quantum_subsystem.charges[4], 0)
        np.testing.assert_array_equal(model.quantum_subsystem.charges,
                                      mol.atom_charges())
        self.assertNotIn('differ', mol.stdout.getvalue())
    ...
    def test_B_dot_x_is_the_response_of_the_potential(self):
        ...
        '''
        mol = butadiene()
        mf = embedding.polarizable(scf.RHF(mol), {'potential': json_file,
    ...
    def test_B_dot_x_keeps_the_shape_and_the_state(self):
        ...
        # a perturbed solve keeps nothing on the subsystem, so there is no
        # cache of them left to have grown
...
class TestElectrostaticEmbedding(unittest.TestCase):
    ...
    def test_hessian_classical_terms_match_finite_differences(self):
        ...
        options = {'potential': potential, 'vdw': {'method': 'LJ'}}
        mol = butadiene()
    ...
    def test_vdw_and_environment_energy_are_unchanged(self):
        ...
        potential = make_vdw_potential()
        mol = butadiene()
        es = embedding.electrostatic(mol, {'potential': potential,
                                           'vdw': {'method': 'LJ'}})
        pe = embedding.polarizable(mol, {'potential': potential,
                                         'vdw': {'method': 'LJ'}})
    ...
    def test_option_validation(self):
        ...
        with self.assertRaises(ValueError) as caught:
            ...
            embedding.electrostatic(mol, {'potential': json_file,
...
@unittest.skipIf(not has_pyframe, 'PyFraME is not available')
class TestPotentialSources(unittest.TestCase):
    '''The potential given as a file, a document, a system or Subsystems.'''

    @classmethod
    def setUpClass(cls):
        cls.system = make_pyframe_system()
        from pyframe.writers import potential_document
        cls.document = potential_document(cls.system)
        cls.path = write_potential(cls.document, 'from_system.json')

    def _energy(self, potential):
        mf = embedding.polarizable(scf.RHF(butadiene()), potential)
        mf.conv_tol = 1e-11
        return mf.kernel()

    def test_every_source_gives_the_same_energy(self):
        reference = self._energy(self.path)
        self.assertAlmostEqual(self._energy(self.system), reference, 10)
        self.assertAlmostEqual(self._energy({'potential': self.system}), reference, 10)
        self.assertAlmostEqual(self._energy({'potential': self.document}), reference, 10)
        self.assertAlmostEqual(self._energy(self.system.to_subsystems()), reference, 10)
        # the document stated its units, and a file without them is refused
        self.assertEqual(self.document['units']['length'], 'bohr')
        unitless = {key: value for key, value in self.document.items() if key != 'units'}
        with self.assertRaisesRegex(ValueError, 'units'):
            embedding.polarizable(butadiene(), write_potential(unitless, 'unitless.json'))

    def test_subsystems_are_not_shared(self):
        '''Two models built from one Subsystems keep their own state.'''
        subsystems = self.system.to_subsystems()
        first = embedding.polarizable(butadiene(), subsystems)
        moved = perturbed_butadiene(0, 0, 0.1)
        second = embedding.polarizable(moved, subsystems)
        self.assertIsNot(first.classical_subsystem,
                         subsystems.classical_subsystems[0])
        self.assertIsNot(first.classical_subsystem, second.classical_subsystem)
        np.testing.assert_allclose(first.quantum_subsystem.coordinates,
                                   butadiene().atom_coords())
        np.testing.assert_allclose(second.quantum_subsystem.coordinates,
                                   moved.atom_coords())

    def test_scanner_with_a_system(self):
        scanner = embedding.polarizable(scf.RHF(butadiene()), self.system).as_scanner()
        moved = perturbed_butadiene(1, 2, 0.05)
        fresh = embedding.polarizable(scf.RHF(moved), self.path)
        fresh.conv_tol = 1e-11
        scanner.conv_tol = 1e-11
        self.assertAlmostEqual(scanner(moved), fresh.kernel(), 9)
